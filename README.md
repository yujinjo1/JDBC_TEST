...
### 시나리오 2: ServerPreparedStatement, 캐시 비활성화 
...
- `Prepared_stmt_count`: PREPARE 후 1 → close 후 0
- `Unwrap`으로 `ServerPreparedStatement`에서 `getServerStatementId()` 확인 시 매번 ID가 달라짐

<br>

### 시나리오 3: ServerPreparedStatement, 캐시 활성화 
...
## 실행 예시: SELECT 쿼리의 캐시 유무 비교

같은 SQL을 3번 반복 실행했을 때의 차이를 보여줍니다.

```sql
SELECT * FROM customer WHERE first_name = ?
```

### 캐시 미사용 (`cachePrepStmts=false`)

```
--- [반복 1] 'MARY' 조회 ---
  PREPARE → Statement ID: 1 발급
  EXECUTE (ID: 1, 바인딩: 'MARY')
  close() → DEALLOCATE (ID: 1 삭제)
  => Prepared_stmt_count: 0

--- [반복 2] 'PATRICIA' 조회 ---
  PREPARE → Statement ID: 2 발급 (새로 생성)
  EXECUTE (ID: 2, 바인딩: 'PATRICIA')
  close() → DEALLOCATE (ID: 2 삭제)
  => Prepared_stmt_count: 0

--- [반복 3] 'LINDA' 조회 ---
  PREPARE → Statement ID: 3 발급 (또 새로 생성)
  EXECUTE (ID: 3, 바인딩: 'LINDA')
  close() → DEALLOCATE (ID: 3 삭제)
  => Prepared_stmt_count: 0
```

> 매번 PREPARE → EXECUTE → DEALLOCATE를 반복하고, ID가 계속 바뀜

### 캐시 사용 (`cachePrepStmts=true`)

```
--- [반복 1] 'MARY' 조회 ---
  PREPARE → Statement ID: 1 발급
  EXECUTE (ID: 1, 바인딩: 'MARY')
  close() → DEALLOCATE 안 함 (캐시에 보관)
  => Prepared_stmt_count: 1

--- [반복 2] 'PATRICIA' 조회 ---
  캐시에서 ID: 1 재사용 (PREPARE 생략)
  EXECUTE (ID: 1, 바인딩: 'PATRICIA')
  close() → 캐시에 보관
  => Prepared_stmt_count: 1

--- [반복 3] 'LINDA' 조회 ---
  캐시에서 ID: 1 재사용 (PREPARE 생략)
  EXECUTE (ID: 1, 바인딩: 'LINDA')
  close() → 캐시에 보관
  => Prepared_stmt_count: 1
```

> 최초 1회만 PREPARE하고, 이후에는 동일한 ID에 바인딩 값만 바꿔서 전송
